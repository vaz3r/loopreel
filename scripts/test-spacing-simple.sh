...
# Simple test script to render slides directly
# Uses the API's internal render endpoint
...
for i in $(seq 0 $((SLIDE_COUNT - 1))); do
  URL="$RENDER_URL/internal/render/$JOB_ID/$i"
  ...
  if [[ -z "$HTML" ]]; then
    warn "  Slide $((i + 1))/$SLIDE_COUNT  FAILED to fetch HTML"
    continue
  fi
  ...
  if [[ -f "$FILEPATH" ]]; then
    ...
    warn "  Slide $((i + 1))/$SLIDE_COUNT  FAILED to capture screenshot"
  fi
done
