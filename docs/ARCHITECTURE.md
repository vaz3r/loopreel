...
└────────┬────────┘  └────────┬─────────┘  └──────────┬───────────┘
        ...
        ▼                ▼           ▼                       ▼      ▼
   ┌─────────┐  ┌──────────────┐ ┌──────────┐  ┌──────────────┐  ┌────────┐
   │ Outbox  │  │worker-ingest │ │worker-   │  │worker-       │  │ R2     │
   │ Relay   │  │              │ │transcribe│  │structure     │  │(Storage)│
   │         │  │ • yt-dlp     │ │         │  │             │  │        │
   │ (in API │  │ • cheerio    │ │ • Whisper │  │ • LLM API  │  │ • audio│
   │ process)│  │ • Puppeteer  │ │ HTTP call │  │ • JSON mode  │  │ • PNG  │
   │         │  │              │ │         │  │ • Zod valid. │  │ • ZIP  │
   │ Polls   │  │ Home server  │ │ Home    │  │             │  │        │
   │ every   │  │ or Oracle    │ │ server  │  │ Oracle VPS  │  │Cloud-  │
   │ 500ms   │  │ concurrency: │ │ or      │  │ concurrency:│  │flare   │
   │         │  │ 3 (I/O)      │ │ Oracle  │  │ 10 (I/O)    │  │S3 API  │
   │ Publishes   │              │ │         │  │             │  │        │
   │ to BullMQ   │ concurrency: │ │ concur: │  │ Retries: 3  │  │Lifecycle:
   │ (idempotent)│ 1 (CPU bound)│ │ 1       │  │ Backoff: exp│  │24h audio│
   │              │              │ │         │  │             │  │delete   │
   │ Retries: max  │ Retries: 2  │ │ Retries:│  │              │  │        │
   │ per-queue    │ Backoff: 5s  │ │ 2       │  │              │  │        │
   │ config       │ Jitter: 1s   │ │ Backoff:│  │              │  │        │
   │              │              │ │ 10s     │  │              │  │        │
   └──────────────┴──────────────┴─────────┴──────────────┴──────────┘
```
